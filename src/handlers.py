...
async def copy_same_time_handler(callback: CallbackQuery, state: FSMContext):
    ...
    if task:
        ...
        database.add(user_id, task["task_text"], today_str, task_time)
...
async def process_time_picker(callback: CallbackQuery, state: FSMContext):
    ...
    if database.check_overlap(user_id, target_date, task_time):
        ...
        if is_editing:
            ...
            msg = f"Time updated to {task_time}!"
        else:
            database.add(user_id, data.get("task_text"), target_date, task_time)
            msg = f"New Task added at {task_time}!"
...
async def force_save_add(callback: CallbackQuery, state: FSMContext):
    ...
    database.add(user_id, data.get("task_text"), current_date, data.get("task_time"))
    await state.clear()
    tasks = database.get(user_id, current_date)
    await callback.message.edit_text(f"Task saved!\nTasks for {current_date}:", reply_markup=keyboards.main_keyboard(tasks, current_date))
    await callback.answer()
